<template>
  <div>
              ...
              <div class="table-responsive">
                ...
                <b-table striped hover :items="data_poin" :fields="fields">
                  <template v-slot:cell(total_poin)="data">
                      <b-badge variant="info">{{ data.item.total_poin }}</b-badge>
                  </template>
    ...
    <b-modal id="modalDetail" size="lg" hide-footer>
      ...
      <b-table striped hocer :items="detail_poin" :fields="fields_detail"></b-table>
    </b-modal>
  </div>
</template>
...
module.exports = {
  ...
  methods: {
    ...
    Detail: function(item) {
      let conf = { headers: { Authorization: "Bearer " + this.key } };
      this.axios.get("/poin_siswa/" + item.id, conf).then(response => {
    ...
    searching: function() {
      ...
      this.axios.post("/poin_siswa/",form, conf)
        .then(response => {
          if (response.data.status == 1) {
            this.$bvToast.hide("loadingToast");
            this.data_poin = response.data.poin;
